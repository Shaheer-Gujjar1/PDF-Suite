    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between rounded-xl border border-border/60 bg-secondary/40 px-4 py-3">
        ...
        <p className="text-xs text-muted-foreground">
          Drag to reorder · Merge in this sequence
        </p>
      </div>
    </div>
  )
...
function SortableMergeCard({ mf, index, thumb, onRemove }: SortableMergeCardProps) {
  ...
  return (
    ...
    >
      {/* Top bar: drag handle + order number + remove */}
      ...
      <div className="mt-2 min-w-0">
        <p className="truncate text-xs font-medium">{mf.file.name}</p>
        <p className="text-[10px] text-muted-foreground">{formatBytes(mf.file.size)}</p>
      </div>
    </div>
  )
}
