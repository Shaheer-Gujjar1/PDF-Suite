...
Task ID: 12
Agent: Super Z (main agent)
Task: Footer cleanup (remove category tagline columns) + add "Blur Face" tool (user request + reference screenshot: selection box with 8 handles over a face, region blurred).

Work Log:
- Footer fix: removed the Image/Organize/Optimize tagline columns from `site-footer.tsx` (single-column brand layout, unused `categories` import + `onNavigate` prop dropped, page.tsx call updated). Header nav untouched. Verified via browser: all 3 taglines gone from footer, 25 tool cards intact.
- Registry: `blur-faces` entry (name "Blur Face", icon ScanFace, category image, accent teal, batch, step 7, NOT locked) + ProcessorType + toolProcessors mapping.
- Worker (`worker-source.ts`): three-pass separable box blur (`boxBlurH/V` sliding-window O(n) per pass, RGB only — alpha preserved for transparent PNGs; 3 box passes ~= gaussian sigma ~= radius). `processors['blur-faces']`: opts.shapes keyed by fileName ({type rect|ellipse, x,y,w,h normalized 0..1, sanitized+clamped}), opts.strength 1..100 -> radius = clamp(round(strength/100 * 0.25 * min(W,H)), 2, 400) so censor strength looks identical across image sizes; per shape: getImageData(bbox) -> boxBlurRGBA -> temp OffscreenCanvas -> ctx.clip(rect|ellipse) -> drawImage back (overlapping shapes stack naturally); source-format preserving encode (jpg white-flatten, webp, else png), `<base>-blurred.<ext>`, note "N area(s) blurred · WxH px" / "no areas · WxH px". Escapes double-backslashed in worker template literal (\\., \\.jpe?g etc.) per Task 11 pitfall.
- View `blur-faces-view.tsx` (~990 lines): 2-col studio. BlurStage: base img hugged by relative wrapper + overlay; per-shape blurred overlay = full-size img clone with CSS `filter: blur(px)` + `clip-path` (inset for rect / ellipse() for face) — cssBlur = workerRadius * (displayedShort/naturalShort) so preview matches output strength; drag-to-draw ghost (Face/Box draw modes, default Face), move by body drag, resize via 8 handles (nw/n/ne/e/se/s/sw/w) with min 2% + clamp to [0,1], pointer capture, Delete-key removes selected, index badge on selected. Sidebar: strength slider (global, 1-100, default 40), Add face/Box quick buttons, area list with select/delete, Clear all, "Apply to all images" (copies shapes to every other file with fresh ids), selected-area card (type switch Face<->Box, px geometry readout from meta dims). Film strip with per-file blur-count badges. Emits `BlurFacesResult { shapes: Record<fileName, BlurShape[]>, strength }`.
- tool-page wiring: image/* input config, INTERACTIVE_TOOLS, result union + isBlurFaces, handleProcess branch (readInput per file; runOptions.shapes + strength), add-more accept, render branch (stableImageFiles).
- E2E (agent-browser): 4 uploads (checker 640x480 PNG, photo 1200x800 JPG, webp 800x600, plain stripes 300x200 PNG). Mouse drag drew ellipse (10.1%,12.0% 39.9%x30.0%) + box; Select-mode body drag moved both (box +4.96%/+2.8%, face +4.77%/+6.36%); SE-handle drag resized box (13.7%x8.1% -> 22.1%x14.2%); Apply-to-all -> all 4 strip badges "2"; Clear all on plain -> 0 + empty-state hint; strength slider clicked to 16%; CSS preview verified (blur(15.5562px) + ellipse clip-path). Run: 4/4 complete, notes "2 areas blurred · <dims>" x3 + "no areas · 300x200". ZIP parsed (4 entries, correct -blurred names); per-item download PNG 47,944 B == ZIP entry size. PIXEL PROOF: plain output bit-exact vs source (0 diffs / 240k bytes); checker output: 18,712/18,712 sampled px outside 62px blur influence BIT-EXACT vs source (0 collateral); 9,575 changed px inside shapes; ellipse-center patch std 2.7 + box-center std 4.6 vs sharp-corner std 76.2 (checker ~127 raw) — content destroyed inside, untouched outside; dims preserved on all outputs. KEY E2E LESSONS: (1) viewport is 1280x577 — pointerdown targets MUST be scrolled into view first (ellipse draw worked only because its start point was above the fold; box draws silently no-opped twice until scrollIntoView); (2) toolbar/sidebar cards appearing after selection shifts layout — re-measure getBoundingClientRect before every mouse gesture; (3) `className.includes('border-primary')` false-positives on `hover:border-primary/80` — use `border-2 border-primary`; (4) verification-script bug: ZIP local-header fields must be read at off+N (fixed-offset reads silently reused entry 1's sizes and aborted the scan after 2 entries — app ZIP was always valid). Radix slider set by clicking track at fraction (value-1)/99.
- Regression: homepage 26 tool cards incl. "Blur Face"; no console errors; `bun run lint` clean; tsc clean for project sources.

Stage Summary:
- ToolForge now ships 29 tools / 22 implemented. Blur Face live: drag-to-draw face/box blur areas (as many as wanted, any position/size), 8-handle resize + drag-move, per-file areas + apply-to-all for batches, size-aware global strength, live CSS-blur preview matching the worker's box-blur radius, batch + per-file/ZIP downloads at original size and format. Footer simplified to brand-only on request.

---
Task ID: 13
Agent: Super Z (main agent)
Task: Meme Maker fixes — (a) "not every font is working" (system-font stack silently fell back to generic sans on devices without Impact/Comic Sans/etc.), (b) "text outside" needed a choice of top / bottom / both caption bars instead of hardcoded top+bottom.

Work Log:
- Root cause (fonts): MEME_FONTS listed OS fonts (Impact, Arial Black, Comic Sans MS, Verdana, Trebuchet MS, Georgia, Times New Roman, Courier New). Canvas silently falls back to one generic sans for any missing family, so several entries rendered identically. Fix: bundle real font files and register them via FontFace in BOTH the main thread (preview canvas) and the worker (OffscreenCanvas output via self.fonts, which is exposed on WorkerGlobalScope in Chromium/Firefox).
- Bundled 12 TTFs in public/fonts/ (2.6MB, lazy-fetched only when the tool runs): Anton, Bangers, Luckiest Guy, Archivo Black (gstatic/google-fonts downloads), Comic Neue 400+700, and local copies DejaVu Sans 400+700, Tinos 400+700, Liberation Mono 400+700. NOTE: /usr/share/fonts/truetype/english/Tinos-*.ttf are broken HTML placeholders — Tinos came from the Google Fonts CSS API gstatic URLs instead; raw.githubusercontent google/fonts path for Tinos is apache/tinos/ (also 404 here, gstatic worked).
- New `src/lib/meme-fonts.ts`: MEME_FONT_DEFS / MEME_FONT_FAMILIES / DEFAULT_MEME_FONT='Anton' / resolveMemeFont() with LEGACY_FONT_MAP (Impact->Anton, Arial Black->Archivo Black, Comic Sans MS->Comic Neue, Verdana+Trebuchet MS->DejaVu Sans, Georgia+Times New Roman->Tinos, Courier New->Liberation Mono) / ensureMemeFonts() — idempotent fetch + document.fonts.add(face) + face.load(), per-file try/catch, caches MemeFontData[] { family, weight, data:ArrayBuffer } for the worker.
- Worker (`worker-source.ts` meme section): MEME_FONT_LEGACY mirror + ensureMemeFontsLoaded(fonts) (FontFace per family/weight, dedup via WORKER_FONTS_ADDED, self.fonts.add + load, silent fallback if FontFace/self.fonts unavailable), awaited at processor start; buildMemeFont now resolves through the map and falls back to ", sans-serif" (Impact removed from the stack). Bars: cfg.bars 'top'|'bottom'|'both' (default both) -> topBar/botBar = bar or 0, H = imgH + topBar + botBar, white fill when topBar+botBar>0 (or jpeg), drawImage(bmp, 0, topBar), note suffix "outside top|bottom|both".
- View (`meme-maker-view.tsx`): fonts from MEME_FONT_FAMILIES (SelectItems render in their own font), default Anton, buildMemeFont uses resolveMemeFont; memeLayout(state, imgH) helper mirrors worker bar math + remapY() for gluing text across layout changes; toggleMode/setBars rewritten on those; addTextAt('top'|'bottom') quick-adds a caption pre-centred in that bar and AUTO-ENABLES the missing bar (switches to 'both' with remap); sidebar gains "Caption bars" Top/Bottom/Both segmented control (outside mode only) and the add-button grid becomes 2-col with "Top text"/"Bottom text" buttons; fontsReady state flips after ensureMemeFonts() and is a MemePreview redraw dep so auto-fit re-measures with real glyphs; helper copy updated ("Every font ships with the app...").
- tool-page.tsx: handleProcess meme branch now also awaits ensureMemeFonts() and passes runOptions.fonts = MemeFontData[] (structured-cloned per task — worker-pool only transfers input buffers, so the same font buffers safely reach every worker).
- E2E (agent-browser): GOTCHA — `agent-browser upload "input[type=file]" @/path` silently creates an EMPTY file (app state shows the name, both <img> loads fail naturalWidth 0, preview stuck on spinner); correct form is a plain path: `agent-browser upload "input[type=file]" /path`. With that fixed: document.fonts.check true for Anton/Bangers/Comic Neue/Tinos; measureText widths differ per family (Anton 203 vs generic 280 vs Luckiest Guy 234 @48px); outside default both = 1200x1056, Top = 928, Bottom = 928, Both = 1056; "Top text" quick-add drew 15,397 dark px in the top bar; "Bottom text" with bars=top auto-switched to both (canvas 1056) and caption landed in the bottom bar; WYSIWYG pixel-compare downloaded test-photo-meme.jpg vs live preview canvas: 1200x1056 both, 99.14% of px within +-8 (mean diff 0.60 — rest = JPEG artifacts + preview-only dashed selection box), caption dark-px counts identical top 15398 / bottom 23900 (proves worker registered the bundled fonts); note "1200x1056 px · 3 texts · outside both"; ZIP contains test-photo-meme.jpg; re-run with bars=top + font switched Comic Neue<->Anton (region hash changes) -> note "1200x928 px · 3 texts · outside top", output 1200x928 with caption in top bar and image reaching the bottom edge (bottom row 37% white = image content, no bar); inside mode back to 1200x800 with bars control hidden. Blob-hook note: window.__bytes also captures the pdf-lib worker-source blob (646KB, starts "/* ====") — filter JPEG/PNG/ZIP signatures before comparing. `bun run lint` clean; no page errors.

Stage Summary:
- Meme Maker fonts now actually work: 8 bundled families (Anton/Bangers/Luckiest Guy/Archivo Black/Comic Neue/DejaVu Sans/Tinos/Liberation Mono) render identically in preview and output on any device, with legacy system-font names mapped so old states keep their look. "Text outside" now offers Top / Bottom / Both caption bars (glue-remap on every switch) plus Top text/Bottom text quick-add buttons that auto-enable their bar. Full WYSIWYG pixel-verified through the worker pipeline; lint clean; screenshot at upload/meme-maker-after.png.

---
Task ID: 14
Agent: Super Z (main agent)
Task: Blur Face — free rotation of blur shapes + invisible shape borders (user request).

Work Log:
- Data model: BlurShape gains `rotation` (degrees, clockwise, around the box centre; x/y/w/h stay the UNROTATED box). Old states without the field default to 0. Worker parse normalizes ((rot%360)+360)%360.
- Worker (`processors['blur-faces']`): per-shape rotated clip — rect: translate(centre) + rotate(θ) + rect(-w/2,-h/2,w,h) + clip, then ctx.setTransform(1,0,0,1,0,0) (clip persists, transform resets) + drawImage of the blurred region aligned to image space; ellipse: ctx.ellipse(cx,cy,rx,ry,rotRad,0,2π) native rotation param + clip + drawImage (no transform dance). Blur region is now the ROTATED hull bbox (w·|cos|+h·|sin| etc.) EXPANDED by radius+2px so box-blur edge sampling near the rotated hull never clamps (rotated corners touch the hull) — this also makes unrotated edges sample real pixels beyond the old bbox, matching the CSS preview better. Outside the clip nothing is drawn (untouched-pixels property preserved).
- View (`blur-faces-view.tsx`): blurred overlay per shape is now a rotated wrapper div (left/top/w/h %, transform rotate(θ), overflow hidden, borderRadius 50% for ellipse) containing a full-size counter-rotated inner <img> (width 100/s.w %, left -(s.x/s.w)·100 %, rotate(-θ) with transformOrigin at the shape centre) — net effect: clip rotated by θ, blurred CONTENT unrotated/aligned with the base image, exactly like the worker. clipPathFor() deleted. Interaction divs get the same rotate(θ) so the 8 handles + new rotate grip ride the shape; NO border classes at all (invisible borders — computed border-width 0px; blurred region itself shows the shape; selection affordance = handles + rotate grip + counter-rotated label badge). Rotate grip = 34px stem + circular RotateCw handle above the top edge (only when selected), cursor grab. Handle cursors rotate with the shape (cursorFor(handle, rot) maps base+rot to nearest of ew/nwse/ns/nesw). Rotation slider (-180..180, step 1) + degree readout added to the Area settings card; helper texts updated.
- Drag math (display-px space, aspect-true): resize converts pointer deltas to the shape-local frame via R(-θ) (ldxP = dx·cos + dy·sin, ldyP = -dx·sin + dy·cos), computes the pointer's BOX-LOCAL position (centre-relative R(-θ) of start pointer + half extents + delta), applies edge min/max per handle, converts the centre shift back via R(θ) in px, then clamps the centre so the rotated bbox (w·|cos|+h·|sin|) stays inside the image. Rotate drag: Δangle = atan2 of (pointer-centre) now vs start, normalized to -180..180, light snap (2°) to 0/±90/180.
- TWO bugs caught by E2E and fixed in the resize math: (1) used the pointer DELTA where the local-frame edge math needs the pointer's absolute local position; (2) forgot the centre-relative -> box-local offset (+o.w/2, +o.h/2) — symptom: dragging 'e' along +u SHRANK w to exactly o.w/2+delta·(1/W). After fixes: dragging 'e' 55px along the 30° local axis gives w 30% -> 40.56% (predicted 40.50%), centre shift keeps the west edge fixed (new left 34.29% matches closed-form to 4 decimals), rotation preserved.
- E2E (agent-browser): borders 0px on selected+unselected; slider drag (radix) needs trusted events — synthetic PointerEvent('pointerdown') on the track DOES set the value but the DOM reflects it only after the next state update (verify via overlay transform, not immediate thumb attr); page scrolls between evals invalidated mouse coords twice — re-measure rects in the same eval as the scroll and use behavior:'instant' (scrollIntoView was async-smooth). Pixel proof on checker PNG (shape 0.3x0.22 at 30°, strength 40 → radius 48): output 640x480; patch-std (41px window) inside rotated rect 0.0 (blurred) vs unrotated-bbox corner wedge 44.2 (sharp); rotated ELLIPSE (rx=96,ry=52.8,30°): lost region (inside unrotated ρ0=76.4, outside rotated ρ1=58.1, φ=330°) std 55.4 sharp, gained region (φ=60°, ρ0=58.1 vs ρ1=76.4) std 40.1 blurred — exact rotation semantics for both clip paths; far/above regions 58-60 (untouched). Rotate grip drag 30°->90° (snapped); unrotated resize regression 40.56%->49.68% (+48px exact); no page errors; lint clean. Screenshot: upload/blur-faces-rotate.png.
...
Task ID: 19
Agent: Super Z (main agent)
Task: Make Watermark PDF exactly like the (perfect) Watermark Image tool — full layer-studio rebuild (user request).

Work Log:
- NEW watermark-pdf-view.tsx (999 lines, faithful port of watermark-images-view.tsx): same layer model (WatermarkPdfLayerState minus 'over' — PDF watermarks always draw on top), same sidebar (Add image/text watermark buttons + layer list with active/delete), same LayerCard (Text/Font/Color, Size auto-fit min(pageW,pageH)/8 pt + slider 8-200 PT, logo % of page width, opacity/rotation sliders, Tile switch + 9-position grid + H/V margins), same floating layer toolbar (reorder up/down + Select), same + FAB with count badge + menu, same film strip (pdf.js page-1 thumbnails instead of img thumbs) + add tile, same hint box. Emit { layers } (null until files+layers exist) drives interactiveReady.
- Live preview: WatermarkPdfPreview renders page 1 via loadPdfJs() (~900px wide, capped 3x) onto an offscreen canvas, then composites layers with the EXACT worker math (pt units, bbox rotation envelope, tile step = bbox + 2*margin, 9-pos corner math). Error state if pdf.js fails.
- worker processors['watermark'] rewritten: multi-layer per file. Fonts map to StandardFonts (Arial/Verdana/Trebuchet->Helvetica, Georgia/Times->TimesRoman, Courier New->Courier), hex->rgb, auto size = max(12, min(pw,ph)/8). pdf-lib anchors text at left-baseline / images at bottom-left and rotates CCW, so stamps are placed via anchor = center - R(-theta)*centerOffset (text v=(tw/2, .36*size), image v=(tw/2, th/2)), rotate: degrees(-theta) => WYSIWYG with the clockwise canvas preview. Logos: PNG/JPEG embedded directly, other formats decoded via createImageBitmap -> OffscreenCanvas -> PNG. Per-doc font/logo caches; opacity per layer; note "N page(s) · M watermark(s)".
- tool-page.tsx: import + WatermarkPdfResult in union, 'watermark' added to INTERACTIVE_TOOLS, isWatermark flag, handleProcess branch (mirrors watermark-images: strips logo/logoUrl, adds logoData ArrayBuffer + logoName, runOptions={layers}), render branch after PdfToExcelView.
- tool-options.tsx: removed 'watermark' from hasOptions + defaultOptions + WatermarkOptions component + SettingsIcon case + unused Stamp import (gotcha: my first removal edit DUPLICATED the component — removed both copies with replace_all).
- tools.ts description: "Stamp text or your logo on every page — font, size, color, opacity, rotation, tiling and 9-position placement with a live page preview. Stack multiple layers."
- bun run lint clean; no page errors.
- E2E (agent-browser, images.pdf 2 large pages 1588x2246pt): studio renders (page thumbs + preview); text layer "CONFIDENTIAL DRAFT" red @ auto-size + logo test-checker.png tiled 25% => preview shows tiled checkers + text (20K reddish px on canvas); output 770.4KB "2 pages · 2 watermarks", rendered via in-page pdf.js: 27,961 reddish px, extraction "IDENTIAL DRAFT" = same left-edge clipping as preview (auto 219pt text wider than page — WYSIWYG, matches image-tool behavior). Run 2 (clean, single text layer "DRAFT COPY" 48pt, rot -30, middle-center): output 760.8KB "2 pages · 1 watermark", extraction "DRAFT COPY" on BOTH pages, matrix d=41.57=48*cos(30) proves exact size+30-degree rotation, anchor at 42%/46% (center-placed). GOTCHAS: (1) agent-browser open to the SAME hash URL does NOT reload the SPA — layers persisted across a fake "reload"; use eval location.reload(). (2) Result-panel download URLs are created lazily on button click — hook URL.createObjectURL (filter type==='application/pdf') then JS-click Download. (3) Synthetic React inputs need the _valueTracker.setValue('') + native setter + input-event trick (worked for color/text); Radix sliders respond to focus + arrow keys only. (4) Stale @refs after DOM swaps silently click wrong elements — re-snapshot after every structural change.
- Screenshots: upload/wmpdf-studio.png, wmpdf-text-layer.png, wmpdf-tiled.png (tiled logo + red text preview), wmpdf-run2b-preview.png (48pt/-30/middle-center card).
...
Task ID: 21
Agent: Super Z (main agent)
Task: Fix Crop PDF selection system — same bug class previously fixed on Crop Images (Tasks 4/5/6): "crop pdf tool has same issues with selection system we previously encountered with crop image tool so need to fix this crop pdf tool in same way".

Work Log:
- Confirmed crop-view.tsx carried ALL the old Crop Images bugs: (1) the substring trap — resize math used dm.includes('w'/'e'/'n'/'s') on the FULL drag-mode string, and 'resize' itself contains 's' and 'e', so EVERY mid-edge handle dragged both axes (an n-swipe moved the top edge, the bottom edge AND the width); (2) mouse events with onMouseLeave=handleMouseUp — drags died at the container edge, no touch support, missable up-events; (3) drag math used async ResizeObserver state instead of a box captured at drag start; (4) getBoundingClientRect()-based coords vs border-exclusive overlay space = 2px offset; (5) unhardened hitTest; (6) BONUS unit bug: the view emitted RAW DISPLAY-PIX rects as PDF points to the worker (opts.crop -> setCropBox), only correct when the container happened to be exactly viewport-sized.
- Rewrote crop-view.tsx interaction layer as a faithful port of the proven crop-images-view system: resizeWithHandles() with direction parsed from the mode SUFFIX (grabW/E/N/S flags) — mid-edge handles strictly single-axis, opposite edge anchored, min-size + container clamps respect anchors (no ratio branch: PDF crop has no presets); createRect() anchor-draw; hardened hitTest() (corner zones shrink to min(HANDLE_SIZE, box/2) on small boxes, side handles accept grabs a few px OUTSIDE the edge so flush x=0/x=cw rects resize instead of re-creating); Pointer Events + setPointerCapture + touchAction:'none' (drags continue outside the preview, up can't be missed); posFromEvent subtracts clientLeft/clientTop and clamps; drag-session ref { mode, anchor, start, box, lastRect } with box measured fresh at pointer-down (immune to mid-drag reflow); useLayoutEffect synchronous measurement + ResizeObserver; rule-of-thirds guides; dead getCursor()/old toPdf/toDisplay removed.
- Crop storage switched to relative (0..1) RelRect (same as Crop Images): selections survive page switches and container rescales; emission converts rel -> PDF points per the page's own dims (pageCrops use each page's dims; all-pages mode uses the current page's), keeping the CropResult worker contract (PDF points) — this also fixes the display-px-as-points bug (output is now exact at any container size). 'Trim margins 10%' now sets rel {0.1,0.1,0.8,0.8}.
- bun run lint clean.
- E2E (agent-browser, scripts/crop-pdf-selection-tests.sh, trusted CDP mouse drags on a generated 2-page 612x792pt test PDF — scripts/make-crop-test-pdf.js): 13/13 behavior checks exact — T1 draw (100,100,200,160); T2 n-handle DIAGONAL swipe -> width stays EXACTLY 200, top follows dy (the reported bug, old code dragged 3 edges); T3a e-handle vertical-only = perfect no-op (rect preserved, not destroyed); T3b e-handle dx -> width only; T4 SE corner both axes NW-anchored; T5/T5b flush x=0 draw + w-handle resize; T6 move keeps dims; T7 e-drag past w-edge clamps at MIN_SIZE=20 without crossing; T8/T8b flush right edge + e-handle grab at x=cw; T9 s-handle bottom-only; T10 NW corner SE-anchored.
- E2E byte-level: drew rect -> Run -> downloaded output -> pdf-lib cropBox/mediaBox = (100, 532, 200, 160) on BOTH pages (exact display->points y-flip math). Per-page mode: crop (50,50,180,140) on p1 + (300,80,100,80) on p2 -> cropBoxes (50,602,180,140) and (300,632,100,80) — each page its own exact region. Relative-rect regression: viewport 560px shrank the container to 482x624 and the overlay scaled to the exact rel-prediction (236.3, 63, 78.8, 63); Trim 10% after re-expand = exactly 61.2/79.2/489.6/633.6. Screenshots: upload/crop-pdf-fixed.png.
- GOTCHAS: (1) React 19 IGNORES untrusted synthetic PointerEvents (dispatchEvent with bubbles:true never reaches onPointerDown) — E2E drags MUST use agent-browser's trusted CDP mouse commands; (2) back-to-back CDP mousemove inputs get coalesced/dropped — sleep ~0.12-0.2s between every move/down/up and duplicate the final move, or drags end mid-way (this also silently breaks pointerdown hit-tests that follow a drag before state flushes); (3) scrollIntoView block:'center' on a 796px-tall container puts its top at y~-38, putting the first drag position UNDER the sticky header — after centering, window.scrollBy so the container top lands at y>=84 and re-measure; (4) sel() must be scoped to .cursor-crosshair container (thumbnail strips also carry border-2).

---
Task ID: 22
Agent: Super Z (main)
Task: New tool "HTML to Image" — render pasted HTML code OR an uploaded .html file into PNG/JPG/WebP, with the two input modes separated by a tabbed system.

Work Log:
- Registry: added tool id `html-to-image` (name "HTML to Image", category image, orange accent, ImageIcon icon, batch false, step 8) to tools.ts; tool count is now 29 (21 locked remain unchanged).
- Processing: added `'html-to-image'` to ProcessorType (types.ts, Step 8 section), mapped it in registry.ts, and added a worker passthrough processor in worker-source.ts — the main thread pre-rasterizes (hidden iframe + html2canvas, same proven pipeline as html-to-pdf), the worker only packages the finished bytes: name passthrough, guessMime by extension, plus a `createImageBitmap` dimensions note ("2560x236 px") that never fails the run.
- New view src/components/tools/html-to-image-view.tsx: PERSISTENT tabbed input (Paste code / Upload file — tabs stay visible so either source can be swapped at any time; pasting clears sourceName, uploading replaces html + sets sourceName), upload dropzone (.html/.htm) with "Currently using <file> (N chars)" chip, live scaled iframe preview (checkerboard backdrop when transparent background is on), and options: Format PNG/JPG/WebP pills, quality slider (50-100%, PNG-locked/disabled), render width desktop 1280 / tablet 768 / mobile 375, scale 1x/2x/3x, background transparent/white (transparent disabled for JPG with hint), output summary line.
- tool-page.tsx wiring: HtmlToImageView import, getInput 'text' case, isHtmlToImage flag, htmlImageConfig state + tool-switch reset, canProcess from config, needsPrepare with "Rendering image…" message, handleProcess branch — full-document HTML is written as-is into the iframe, snippets get a minimal boilerplate wrap; ensureHtml2Canvas() helper (robust: polls up to 5s instead of html-to-pdf's blind 500ms wait, same CDN URL for cache reuse); capture at width×contentH with backgroundColor null (transparent) or #ffffff; canvas.toBlob(format, quality) — the ACTUAL blob.type decides extension/mime so Safari's WebP→PNG silent fallback can't produce mislabeled files; output named after the uploaded file (slugified) or 'html-to-image'; mode 'per-file' through processor 'html-to-image'; render branch before isHtmlToPdf; action-bar status text; ToolOptions guard.
- HEIGHT FIX (found via console probe): documentElement.scrollHeight is floored at the iframe viewport (800px) — short snippets captured with dead whitespace below (2560x1600 instead of ~2560x300). Fix: when root scrollHeight === clientHeight (pure viewport floor), use the BODY's true content height (scrollHeight vs getBoundingClientRect max); captures now measure exactly the content (1280x118 probe result).
- bun run lint clean; dev.log clean.
- E2E (agent-browser, scripts/html-to-image-tests.sh): 12/12 PASS — home card present; both tabs render; paste -> live preview + options + Run enable; PNG run -> Download-hooked blob magic bytes 89 50 4e 47 + mime image/png; worker dims note rendered; Upload tab -> DataTransfer File 'my-card.html' -> chip shown -> Run -> result 'my-card.png'; JPG run -> ff d8 ff + image/jpeg -> 'my-card.jpg'. Screenshot upload/html-to-image-final.png.
- GOTCHAS: (1) result blob URLs are created LAZILY on Download-button click — E2E byte checks must hook URL.createObjectURL then click the per-item Download button (per-item button = svg.lucide-download + text 'Download'; the panel header Download-All button's label is ALSO exactly 'Download' when only one output exists — discriminate by icon); (2) wait for done state via per-item Download button count, NOT the item label (labels render while queued); (3) agent-browser returns eval'ed JSON strings with escaped quotes — tr -d '\\' before grep; (4) worker-pool blob URLs (application/javascript) also flow through the createObjectURL hook — filter by image/* prefix.

Stage Summary:
- HTML to Image shipped and E2E-verified 12/12: tabbed Paste-code/Upload-file input, full-doc + snippet support, PNG/JPG/WebP output with quality, 375/768/1280 render widths, 1x-3x scale, transparent/white background, content-hugging capture height, correct magic bytes + mime + file naming (uploaded file name carried through), worker dimension notes. 29 tools total; no locked tools touched.

---
Task ID: 23
Agent: Super Z (main)
Task: New tool "Photo Editor" (per user's iLoveIMG reference screenshot) — description explicitly states it is JUST A BASIC photo editor.

Work Log:
- Registry: tool id `photo-editor` (name "Photo Editor", description "Just a basic photo editor — crop, resize, rotate and flip, tune filters and light, draw, add text, shapes and frames. Simple edits, 100% in your browser.", category image, orange, Brush icon, batch false, step 8). Tool count now 30; no locked tools touched.
- Processing: `'photo-editor'` added to ProcessorType + registry; worker processor = same packaging passthrough as html-to-image (main thread renders, worker packages bytes + createImageBitmap dims note).
- New view src/components/tools/photo-editor-view.tsx (~1540 lines): 8-mode toolbar (Filter & light / Crop / Transform / Resize / Draw / Text / Shapes / Frame) + Undo/Redo/Reset-all. Architecture: destructive base canvas (baseRef) + live overlays (strokes/shapes/texts in natural px) + render-time adjustments (ctx.filter CSS string: brightness/contrast/saturate/sepia/blur, blur scaled by k) + render-time frame/corner-radius. Preview render at fit-scale k with DPR-size canvas; export renders the same pipeline at k=1 into PNG via toBlob, debounced 500ms emission to onChange (PhotoEditorResult { fileName: <orig>-edited.png, data, w, h }); Run sends bytes through the worker passthrough (WYSIWYG guaranteed by construction).
- Crop: faithful port of the proven crop-images selection system (resizeWithHandles with SUFFIX-parsed grab edges, createRect anchor-draw, fitRectToRatio, hardened hitTest, pointer capture + touchAction none, dark mask via box-shadow, ratio presets Free/Original/1:1/4:3/16:9, Full area, min-size discard). Crop overlays the PHOTO area (offset by frame) inside an overflow-hidden wrapper. Apply bakes crop and shifts overlays; Apply crop/resize/transform remap overlay coords (rotate90 point map, flip map, scale map incl. font sizes).
- Draw: freehand strokes (color, 2-60px brush, round caps, distance-throttled points). Text: click-to-place (auto size = 8% height), panel edits content/size(8-300)/color/bold/italic, drag to move, baseline anchor, hit box via offscreen measureText at 100px. Shapes: rect/ellipse/line drag-create, filled toggle, stroke width, select+move, delete. Frame: border width 0-100 + color + corner radius 0-200 (roundRect clips, transparent corners in PNG). Adjust: 5 sliders + presets (Original/Mono/Sepia/Vivid/Soft), history pushed on slider onValueCommit via onPointerDownCapture pre-snapshot.
- Undo/redo: 25-deep snapshots { base: cloned canvas, overlays, adj, frame }; depth mirrored into histDepth STATE for reactive disabled buttons (new react-hooks/refs rule FORBIDS ref.current reads during render — baseW/baseH also moved to `dims` state set by load/apply ops).
- BUG FOUND BY E2E (T7): restore() did not update dims after undo/redo — UI showed stale rotated dims AND the export drew the restored canvas into stale dimensions (T8 output was 180x240 instead of 240x180). Fixed: restore() now setDims from the restored canvas.
- bun run lint clean (after moving measureTextW above renderAt for the react-hooks access-order rule; an over-broad Edit briefly mangled hitOverlay's header — repaired).
- E2E (agent-browser, scripts/photo-editor-tests.sh): 12/12 PASS — home card; 8-mode toolbar after in-page-generated test PNG upload; Sepia preset (Warmth 80%); crop default 60% applies to exactly 240x180 (verified via Resize panel "Current size"); TRUSTED CDP MOUSE draw stroke — center canvas pixel sampled as rgba(239,68,68) = the exact brush color; click plants text + panel shows 'Your text'; Rotate right -> 180x240; Undo -> 240x180; Run -> download-hooked PNG magic 89 50 4e 47 0d 0a 1a 0a with IHDR w/h read via DataView; result named test-photo-edited.png. Screenshot upload/photo-editor-final.png.
- GOTCHAS: (1) react-hooks v6 eslint: no ref.current reads in render (derive dims/depth into state), useCallback bodies must declare vars BEFORE use (measureTextW), no hooks after early returns; (2) "Current size" lives only in the Resize panel — dim assertions must switch modes first; (3) same CDP-mouse/sleep rules as before for canvas pointer drags; (4) setState updaters must stay pure (no ref writes inside — moved futureRef reset out).

Stage Summary:
- Photo Editor shipped and E2E-verified 12/12: basic-editor scope exactly as requested (description says "Just a basic photo editor"), crop with the proven selection system, resize, rotate/flip, filter & light presets+sliders, draw, text, shapes, frame+rounded corners, full undo/redo/reset, WYSIWYG PNG export through the worker. 30 tools total; all 21 locked tools untouched.
